...
from smef.double_range_slider import QRangeSlider
from smef.utils import reverse_convert, converter
...
class SessionViewer(QWidget):
    def __init__(self, **kwargs):
        super().__init__()
        loadUi(os.path.join(os.path.dirname(__file__), 'session_viewer.ui'), self)
        self.viewer_custom_plot = CustomPlot()
        self.plotter_layout.addWidget(self.viewer_custom_plot)
        self.setWindowTitle('Просмотр сеанса')
        self.setWindowIcon(QtGui.QIcon(os.path.join(os.path.dirname(__file__), '../icon/engeneering.ico')))
        ...
        self.units = 'В/м'
        self.units_mode = 0
        self.plotter_layout.addWidget(self.slider)
        ...
        self.viewer_custom_plot.legend.clear()
        self.data_len = len(self.np_data[0])
        self.viewer_custom_plot.main_plot_item.setLimits(yMin=-10000, yMax=10000, xMin=self.np_data[0][0] - 50000,
                                                   xMax=self.np_data[0][-1] + 50000)
        self.viewer_custom_plot.original_data = self.np_data
        self.viewer_custom_plot.data = self.np_data
        ...
        if self.data is not None:
            for i in range(len(self.np_data) - 1):
                if self.np_data[i+1] is not None:
                    # boolean mask for self.viewer_custom_plot.data_line array
                    [b for a, b in zip(self.connected_sensors, self.viewer_custom_plot.data_line) if a][i].setData(self.np_data[0], self.np_data[i+1])
                    if init_flag:
                        self.viewer_custom_plot.legend.addItem(self.viewer_custom_plot.data_line[self.indices[i]], "Датчик " + str(self.indices[i] + 1))
                    # self.viewer_custom_plot.data_line[i].setData(np_data[0], np_data[i+1])
            self.viewer_custom_plot.main_plot_item.enableAutoRange(axis='y', enable=True)
            self.viewer_custom_plot.main_plot_item.enableAutoRange(axis='x', enable=True)
    ...
    def units_update(self):
        try:
            if self.sender().text() != self.units:
                ...
                if self.units_rbutton1.isChecked():
                    ...
                    # self.viewer_custom_plot.convert_plot_data(mode=0)

                    self.viewer_custom_plot.main_plot_item.enableAutoRange(axis='y', enable=True)
                    self.viewer_custom_plot.main_plot_item.enableAutoRange(axis='x', enable=True)
                    if self.current_locale == 'ru':
                        self.units = 'В/м'
                        self.viewer_custom_plot.left_axis.labelUnits = "В/м"
                        self.viewer_custom_plot.russian_labels['units'] = "В/м"
                    else:
                        self.units = 'V/m'
                        self.viewer_custom_plot.left_axis.labelUnits = "V/m"
                        self.viewer_custom_plot.english_labels['units'] = "V/m"
                    self.viewer_custom_plot.left_axis.label.setHtml(self.viewer_custom_plot.left_axis.labelString())
                ...
                if self.units_rbutton2.isChecked():
                    ...
                    if self.current_locale == 'ru':
                        self.units = 'дБмкВ/м'
                        self.viewer_custom_plot.left_axis.labelUnits = "дБмкВ/м"
                        self.viewer_custom_plot.russian_labels['units'] = "дБмкВ/м"
                    else:
                        self.units = 'dBµV/m'
                        self.viewer_custom_plot.left_axis.labelUnits = "dBµV/m"
                        self.viewer_custom_plot.english_labels['units'] = "dBµV/m"
                    self.viewer_custom_plot.left_axis.label.setHtml(self.viewer_custom_plot.left_axis.labelString())
                ...
                if self.units_rbutton3.isChecked():
                    self.last_units = copy.copy(self.units)
                    # self.viewer_custom_plot.convert_plot_data(mode=2)
                    self.viewer_custom_plot.main_plot_item.enableAutoRange(axis='y', enable=True)
                    self.viewer_custom_plot.main_plot_item.enableAutoRange(axis='x', enable=True)
                    if self.current_locale == 'ru':
                        self.units = 'Вт/м²'
                        self.viewer_custom_plot.left_axis.labelUnits = "Вт/м²"
                        self.viewer_custom_plot.russian_labels['units'] = "Вт/м²"
                    else:
                        self.units = 'W/m²'
                        self.viewer_custom_plot.left_axis.labelUnits = "W/m²"
                        self.viewer_custom_plot.english_labels['units'] = "W/m²"
                    self.viewer_custom_plot.left_axis.label.setHtml(self.viewer_custom_plot.left_axis.labelString())
                    if self.last_units in ('дБмкВ/м', 'dBµV/m'):
                        self.viewer_norma_val_spinbox.setValue(reverse_convert(self.viewer_norma_val_spinbox.value(), mode=2))
                        self.viewer_norma_val_spinbox.setValue(self.viewer_norma_val_spinbox.value() / 377)
                ...
                self.viewer_custom_plot.change_locale(self.current_locale)
        except Exception as ex:
            logger.error(ex)

        if self.units_rbutton1.isChecked():
            # self.viewer_custom_plot.convert_plot_data(mode=0)
            self.np_data = np.copy(self.viewer_custom_plot.original_data)
            self.viewer_custom_plot.data = self.np_data
            self.update_viewer()
        ...
        if self.units_rbutton2.isChecked():
            ...
            self.viewer_custom_plot.data = self.np_data
            self.update_viewer()
            # self.viewer_custom_plot.convert_plot_data(mode=1)
            if self.current_locale == 'ru':
                self.viewer_custom_plot.left_axis.labelUnits = "дБмкВ/м"
    ...
    def update_plot(self, value):
        ...
        for i in range(len(self.np_data) - 1):
            if self.np_data[i+1] is not None:
                # boolean mask for self.viewer_custom_plot.data_line array
                [b for a, b in zip(self.connected_sensors, self.viewer_custom_plot.data_line)
    ...
    def set_title(self):
        ...
        item = self.viewer_custom_plot.getPlotItem()
        if self.viewer_custom_plot.theme == 'dark':
    ...
    def copy_data(self):
        x1_timestamp = int(self.viewer_custom_plot.pw.visibleRange().getCoords()[0])
        x2_timestamp = int(self.viewer_custom_plot.pw.visibleRange().getCoords()[2])
        data = self.data.copy()
    ...
    def copy_image(self):
        try:
            ...
            exporter = pg.exporters.ImageExporter(self.viewer_custom_plot.main_plot_item)
    ...
    def norma_checked(self):
        ...
        # self.customplot.addItem(line)
        val = self.viewer_norma_val_spinbox.value()
        state = self.viewer_norma_checkbox.checkState()
        if self.viewer_custom_plot.infinite_line is not None:
            self.viewer_custom_plot.main_plot_item.removeItem(self.viewer_custom_plot.infinite_line)
            self.viewer_custom_plot.infinite_line = None
        if state == 2:
            self.viewer_custom_plot.infinite_line = self.viewer_custom_plot.main_plot_item.addLine(
    ...
    def hide_line_plot(self):
        self.viewer_custom_plot.data_line[0].setVisible(self.s1_legend_checkbox.isChecked())
        self.viewer_custom_plot.data_line[1].setVisible(self.checkBos2_legend_checkbox.isChecked())
        self.viewer_custom_plot.data_line[2].setVisible(self.s3_legend_checkbox.isChecked())
        self.viewer_custom_plot.data_line[3].setVisible(self.s4_legend_checkbox.isChecked())
        self.viewer_custom_plot.data_line[4].setVisible(self.s5_legend_checkbox.isChecked())
        self.viewer_custom_plot.legend.update()

    def on_off_markers(self, state):
        if state:
            self.viewer_custom_plot.display_data_under_mouse = True
            self.viewer_custom_plot.cursor_vLine.setVisible(True)
            self.viewer_custom_plot.cursor_hLine.setVisible(True)
            self.viewer_custom_plot.marker_label.setVisible(True)
        else:
            self.viewer_custom_plot.display_data_under_mouse = False
            self.viewer_custom_plot.cursor_vLine.setVisible(False)
            self.viewer_custom_plot.cursor_hLine.setVisible(False)
            self.viewer_custom_plot.marker_label.setVisible(False)


if __name__ == '__main__':
    app = QApplication([])
    main_window = SessionViewer()
    main_window.show()
    app.exec_()
